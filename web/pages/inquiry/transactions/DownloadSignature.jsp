...
<%
    ...
    String signatureName = request.getParameter("hdnFileName");
    String signaturePath = request.getParameter("hdnFilePath");

    signatureName = signatureName.replaceAll(LCPL_Constants.file_ext_type_pdf, LCPL_Constants.file_ext_type_sig);
    signaturePath = signaturePath.replaceAll(LCPL_Constants.file_ext_type_pdf, LCPL_Constants.file_ext_type_sig);

    if (!signaturePath.endsWith(LCPL_Constants.file_ext_type_sig))
    {
        signaturePath = signaturePath + LCPL_Constants.file_ext_type_sig;
    }
    ...
    reportFile = new File(signaturePath);
    ...
    if (reportFile.exists())
    {
        ...
        try
        {
            ...
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_user_inquiry_transaction_inward_download, "| File Id - " + reportFile.getName() + ", File Type - .sig, Download Status. - Successful , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_user_inquiry_transaction_inward_download, "| File Id - " + reportFile.getName() + ", File Type - .sig, Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
    ...
    {
        DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_user_inquiry_transaction_inward_download, "| File Id - " + reportFile.getName() + ", File Type - .sig, Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>

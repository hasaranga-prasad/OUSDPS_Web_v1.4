...
//    String reportPath = request.getParameter("hdnFilePath");
    ...
    File fTransTypes = null;
    BufferedInputStream bufIn = null;
    ServletOutputStream servOut = null;
    int reportSize = -1;

    fTransTypes = PropertyLoader.getInstance().getTransactionFile();

    if (fTransTypes.exists())
    {
        reportSize = (int) fTransTypes.length();
        ...
        try
        {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + fTransTypes.getName());
            response.setContentLength(reportSize);

            FileInputStream input = new FileInputStream(fTransTypes);
            ...
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_transaction_type_file, "| File Id - " + fTransTypes.getName() + ", File Type - .XML, Download Status. - Successful , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_transaction_type_file, "| File Id - " + fTransTypes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
    ...
    {
        DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_transaction_type_file, "| File Id - " + fTransTypes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>

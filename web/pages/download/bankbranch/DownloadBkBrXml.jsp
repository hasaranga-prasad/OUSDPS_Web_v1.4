...
//    String reportPath = request.getParameter("hdnFilePath");
    ...
    bkbrXml = PropertyLoader.getInstance().getBankBranchXmlFile();

    if (bkbrXml.exists())
    {
        reportSize = (int) bkbrXml.length();
        ...
        try
        {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + bkbrXml.getName());
            response.setContentLength(reportSize);

            FileInputStream input = new FileInputStream(bkbrXml);
            ...
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_bank_branch_xml, "| File Id - " + bkbrXml.getName() + ", File Type - .XML, Download Status. - Successful , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_bank_branch_xml, "| File Id - " + bkbrXml.getName() + ", File Type - .XML, Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
    ...
    {
        DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_bank_branch_xml, "| File Id - " + bkbrXml.getName() + ", File Type - .XML, Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>

...
//    String reportPath = request.getParameter("hdnFilePath");
    ...
    File fReturnTypes = null;
    BufferedInputStream bufIn = null;
    ServletOutputStream servOut = null;
    int reportSize = -1;

    fReturnTypes = PropertyLoader.getInstance().getReturnFile();

    if (fReturnTypes.exists())
    {
        reportSize = (int) fReturnTypes.length();
        ...
        try
        {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + fReturnTypes.getName());
            response.setContentLength(reportSize);

            FileInputStream input = new FileInputStream(fReturnTypes);
            ...
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_return_type_file, "| File Id - " + fReturnTypes.getName() + ", File Type - .XML, Download Status. - Successful , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_return_type_file, "| File Id - " + fReturnTypes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
    ...
    {
        System.out.println(fReturnTypes.getPath());
        
        DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_return_type_file, "| File Id - " + fReturnTypes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>
...
<html>
    <head>
        ...
        <title>OUSDPS Web - Download Return Types</title>
    </head>
    <body>
    </body>
</html>

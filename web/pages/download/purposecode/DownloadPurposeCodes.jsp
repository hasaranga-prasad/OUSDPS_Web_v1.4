...
//    String reportPath = request.getParameter("hdnFilePath");
    ...
    File fPurposeCodes = null;
    BufferedInputStream bufIn = null;
    ServletOutputStream servOut = null;
    int reportSize = -1;

    fPurposeCodes = PropertyLoader.getInstance().getPurposeCodeFile();

    if (fPurposeCodes.exists())
    {
        reportSize = (int) fPurposeCodes.length();
        ...
        try
        {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + fPurposeCodes.getName());
            response.setContentLength(reportSize);

            FileInputStream input = new FileInputStream(fPurposeCodes);
            ...
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_purpose_code_file, "| File Id - " + fPurposeCodes.getName() + ", File Type - .XML, Download Status. - Successful , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_purpose_code_file, "| File Id - " + fPurposeCodes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
    ...
    {
        DAOFactory.getLogDAO().addLog(new Log(LCPL_Constants.log_type_download_purpose_code_file, "| File Id - " + fPurposeCodes.getName() + ", File Type - .XML, Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + userName + " (" + userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>
